...
public final class CustomActivityOnCrash {
    ...
    private static final String SHARED_PREFERENCES_APP_BUILD_TIME = "app_build_time";
     ...
     */
    public static void install(Context context, long buildTime) {
        try {
            setAppBuildTime(context, buildTime);
            if (context == null) {
    ...
    private static void setAppBuildTime(Context context, long buildTime) {
        context.getSharedPreferences(SHARED_PREFERENCES_FILE, Context.MODE_PRIVATE).edit().putLong(SHARED_PREFERENCES_APP_BUILD_TIME, buildTime).commit();
    }
    ...
    public static int getDefaultErrorActivityDrawableIdFromIntent(Intent intent) {
        return intent.getIntExtra(CustomActivityOnCrash.EXTRA_IMAGE_DRAWABLE_ID, R.mipmap.ic_launcher);
    }
...
//        killCurrentProcess();
    ...
    private static String getBuildDateAsString(Context context, DateFormat dateFormat) {
        ...
        try {
            ...
            zf.close();*/
            long buildTime = context.getSharedPreferences(SHARED_PREFERENCES_FILE, Context.MODE_PRIVATE).getLong(SHARED_PREFERENCES_APP_BUILD_TIME, -1);
            if (buildTime != -1) {
                buildDate = dateFormat.format(new Date(buildTime));
            } else {
                buildDate = "Unknown";
            }
        } catch (Exception e) {
