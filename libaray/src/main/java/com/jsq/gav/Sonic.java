...
public class Sonic {
    ...
    // is returned, which is not an error condition.
//    int framesToRead = Math.min(buffer.remaining() / numChannels, numOutputSamples);
//        buffer.put(outputBuffer, 0, framesToRead * numChannels);
//    numOutputSamples -= framesToRead;

    public int readShortFromStream(
            short samples[],
            int maxSamples) {
        int numSamples = numOutputSamples * numChannels;
        int remainingSamples = 0;
    ...
    // we fail to resize an input or output buffer.  Also speed the output by the volume.
    ...
    public void queueInput(ShortBuffer buffer) {
        int framesToWrite = buffer.remaining() / numChannels;
        int bytesToWrite = framesToWrite * numChannels * 2;
        inputBuffer = ensureSpaceForAdditionalFrames(inputBuffer, numInputSamples, framesToWrite);
        buffer.get(inputBuffer, numInputSamples * numChannels, bytesToWrite / 2);
        numInputSamples += framesToWrite;
        processStreamInput();
    }

    public int getOutput(ShortBuffer buffer) {
        int framesToRead = Math.min(buffer.remaining() / numChannels, numOutputSamples);
        buffer.put(outputBuffer, 0, framesToRead * numChannels);
        numOutputSamples -= framesToRead;
        System.arraycopy(
                outputBuffer,
                framesToRead * numChannels,
                outputBuffer,
                0,
    ...
    private short[] ensureSpaceForAdditionalFrames(
        ...
        int currentCapacityFrames = buffer.length / numChannels;
        if (frameCount + additionalFrameCount <= currentCapacityFrames) {
            ...
            return Arrays.copyOf(buffer, newCapacityFrames * numChannels);
        }
    }
