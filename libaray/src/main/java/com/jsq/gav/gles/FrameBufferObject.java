...
public class FrameBufferObject {
    ...
    public void setup(final int width, final int height) {
        ...
        Log.d(TAG, "GLFrameBufferObject setup");
        ...
        try {
            ...
            renderBufferId = args[0];
            GLES20.glBindRenderbuffer(GL_RENDERBUFFER, renderBufferId);
            GLES20.glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, width, height);
            GLES20.glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, renderBufferId);

            GLES20.glGenTextures(args.length, args, 0);
            textureId = args[0];  // 这个纹理作为framebuffer的颜色缓冲区(GL_COLOR_ATTACHMENT0), 也就是视频流输出
